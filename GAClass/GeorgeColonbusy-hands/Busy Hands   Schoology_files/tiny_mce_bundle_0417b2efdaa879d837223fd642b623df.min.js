...
E=G.clientHeight;if((F<D.y)||(F+E>D.y+D.h)){C.body.scrollTop=F<D.y?F:F-D.h+25}},0)}},_legacySupport:function(){var f=this,e=f.editor;e.addCommand("mcePasteWord",function(){e.windowManager.open({file:f.url+"/pasteword.htm",width:parseInt(b(e,"paste_dialog_width")),height:parseInt(b(e,"paste_dialog_height")),inline:1})});if(b(e,"paste_text_use_dialog")){e.addCommand("mcePasteText",function(){e.windowManager.open({file:f.url+"/pastetext.htm",width:parseInt(b(e,"paste_dialog_width")),height:parseInt(b(e,"paste_dialog_height")),inline:1})})}e.addButton("pasteword",{title:"paste.paste_word_desc",cmd:"mcePasteWord"})}});tinymce.PluginManager.add("paste",tinymce.plugins.PastePlugin)})();var sTinymceOpts={};Drupal.behaviors.s_tinymce=function(e){if(!Drupal.settings.s_tinymce){return}var d=Drupal.settings.s_tinymce;if(!d.editors||typeof d.editors!="object"){return}if(tinyMCE.activeEditor){var f=tinyMCE.activeEditor.editorId;if(f&&f!="edit-tme-textarea"){var h=false;$.each(d.editors,function(k,l){if($.inArray(f,l)==-1){h=true
}});if(!h){return}}}if($("#tinymce-toggle-html",e).hasClass("tinymce-toggle-active")){return}var b=[];$.each(d.editors,function(k,l){$.each(l,function(m,n){var o=$("#"+n);if(!o.hasClass("sTinymce-processed")){b.push(n);o.addClass("sTinymce-processed")}})});if(!b.length){return}var g=$(".tinymce-html-btn",e),a=$(".tinymce-visual-btn",e);g.each(function(){$(this).unbind("click").bind("click",function(){if(tinyMCE.activeEditor==null){return}var n=$(this).attr("editor_id");var l=$(this).parents("form");var m=$("textarea#"+n,l);tinyMCE.execCommand("mceRemoveControl",false,n);$("#"+n).focus();$("#tinymce-toolbar-placeholder",l).css("display","block");m.css("visibility","visible").focus();$(".tinymce-ext-buttons",l).hide();$("#topic-content-select-wrapper",l).hide();if(m.closest(".s-tme-with-attachments")){var k=m.siblings(".s-tme-attachment-row");if(!k.length){var k=$("<div/>").addClass("s-tme-attachment-row");k.insertAfter(m)}k.show()}$(this).addClass("tinymce-toggle-active");$(this).siblings(".tinymce-visual-btn").removeClass("tinymce-toggle-active");
sPopupsResizeCenter()})});a.each(function(){$(this).unbind("click").bind("click",function(){var m=$(this).attr("editor_id");var k=$(this).parents("form");if(tinyMCE.activeEditor&&tinyMCE.activeEditor.editorId==m){return}var l=$("textarea#"+m,k);tinyMCE.execCommand("mceAddControl",true,m);$("#tinymce-toolbar-placeholder",k).hide();l.css("visibility","hidden");$(".tinymce-ext-buttons",k).show();$("#topic-content-select-wrapper",k).show();if(l.closest(".s-tme-with-attachments")){l.siblings(".s-tme-attachment-row").hide()}sTinymcePopup.updateEditorHeight();$(this).addClass("tinymce-toggle-active");$(this).siblings(".tinymce-html-btn").removeClass("tinymce-toggle-active");sPopupsResizeCenter()})});g.add(a).tipsy({gravity:"s",title:function(){return $(this).attr("original-title")}});$(".scontent-image-insert",e).each(function(){$(this).unbind("click").bind("click",function(){tinyMCE.activeEditor.execCommand("sContentImageInsert")})});$(".scontent-media-insert",e).each(function(){$(this).unbind("click").bind("click",function(){tinyMCE.activeEditor.execCommand("sContentMediaInsert")
})});$(".scontent-formula-insert",e).each(function(){$(this).unbind("click").bind("click",function(){tinyMCE.activeEditor.execCommand("sContentFormulaInsert")})});$(".scontent-latex-insert",e).each(function(){$(this).unbind("click").bind("click",function(){tinyMCE.activeEditor.execCommand("sContentLatexInsert")})});$(".s-tinymce-loader",e).each(function(){$(this).remove()});if($("#"+b[0],e).parents(".popups-box").length>0){$(document).data("sTinymcePopupActiveEditors",b.join(","));sTinymceInit({mode:"none"});$(document).unbind("popups_open_path_done.s_tinymce").bind("popups_open_path_done.s_tinymce",function(){var n=$(document).data("sTinymcePopupActiveEditors");if(!n){return}var k=Popups.activePopup();var m=$("#"+n).parents(".popups-box");if(k&&k.id&&k.id!=m.attr("id")){return}tinyMCE.onAddEditor.add(function(q,p){if(p.id==n){p.onInit.add(function(){sPopupsResizeCenter()})}});tinyMCE.execCommand("mceRemoveControl",false,"edit-tme-textarea");tinyMCE.execCommand("mceAddControl",false,n);
var o=sUserGetUISettings("rte","view_mode")=="standard"?"standard":"lite";var l=false;$.each(b,function(s,r){var p=$("#"+r,e);if(p.hasClass("s-tinymce-view-modes")){l=true;var u=$('<span class="s-tinymce-toggle-view" />'),t=Drupal.t("Expand"),q=Drupal.t("Minimize");u.appendTo(p.parent());u.click(function(){sTinymcePopup.setView(sTinymcePopup.getView()=="standard"?"lite":"standard")}).tipsy({gravity:"s",title:function(){return sTinymcePopup.getView()=="standard"?q:t}})}});if(l){sTinymcePopup.setView(o,false)}});$(document).bind("popups_before_submit",function(p,r,k,l){tinyMCE.triggerSave();var o=new Array();var n=new Array();$.each(tinyMCE.editors,function(s,t){o.push(t.editorId)});if(o.length==0){return}$("textarea",k).each(function(){var t=$(this).attr("id");if($.inArray(t,o)!=-1){var s=$(this).attr("name");n.push(s)}});if(n.length==0){return}var q=tinyMCE.get(tinyMCE.activeEditor.editorId).getContent();$.each(r,function(t,s){if($.inArray(s.name,n)!=-1){r[t].value=q}});var m=$(document).data("sTinymcePopupActiveEditors");
tinyMCE.execCommand("mceRemoveControl",true,m)});$(document).bind("popups_form_success_notdone",function(m,l,n){var k=$(document).data("sTinymcePopupActiveEditors");if(!k){return}tinyMCE.execCommand("mceAddControl",true,k)});$(document).bind("popups_before_remove",function(l,k,m){if(typeof k=="object"&&k.id&&typeof tinyMCE=="object"&&tinyMCE.activeEditor&&tinyMCE.activeEditor.s_tinymce_popup==k.id){var n=tinyMCE.activeEditor.editorId;tinyMCE.execCommand("mceFocus",false,n);tinyMCE.execCommand("mceRemoveControl",false,n);$(document).data("sTinymcePopupActiveEditors",false);Drupal.settings.s_tinymce={}}})}else{$.each(b,function(l,n){var m=tinyMCE.get(n);if(typeof m=="object"){tinyMCE.remove(m)}});sTinymceInit({elements:b.join(",")})}};var sTinymceInit=function(b){var e=typeof Drupal.settings.s_tinymce_alt_opts!="undefined"?Drupal.settings.s_tinymce_alt_opts:{};var d=typeof Drupal.settings.s_tinymce!="undefined"&&typeof Drupal.settings.s_tinymce.language!="undefined"?Drupal.settings.s_tinymce.language:"en";
var a="full";if(typeof e.toolbar!="undefined"){a=e.toolbar}else{if(typeof b.toolbar!="undefined"){a=b.toolbar}}if(typeof a=="object"){a=a[0]}sTinymceOpts={mode:"exact",theme:"advanced",language:d,cache_string:Drupal.settings.s_tinymce&&Drupal.settings.s_tinymce.cache?Drupal.settings.s_tinymce.cache:"",elements:"",relative_urls:false,remove_script_host:true,paste_retain_style_properties:"all",paste_remove_styles_if_webkit:false,setup:function(g){g.onInit.add(function(h){$(".mceButton","#"+h.id+"_tbl").tipsy({gravity:"s",title:function(){return $(this).attr("original-title")}});var k=$("#"+g.editorId).parents(".popups-box");g.s_tinymce_popup=k.attr("id")})},theme_advanced_layout_manager:"SimpleLayout",theme_advanced_buttons1:"",theme_advanced_buttons2:"",theme_advanced_buttons3:"",theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",theme_advanced_statusbar_location:"bottom",theme_advanced_resizing:true,theme_advanced_resize_horizontal:false,theme_advanced_resizing_use_cookie:false,plugins:"",apply_source_formatting:true,extended_valid_elements:"label[id|class|style],span[id|class|style|cluetip],object[classid|codebase|width|height|align],param[name|value],embed[quality|type|pluginspage|width|height|src|align],iframe[src|width|height|name|align|scrolling|frameborder|style|allowfullscreen],img[src|alt|width|height|style|id|name|formula|class|align|title|border|cluetip],td[id|class|style|title|lang|dir|onclick|ondblclick|onmousedown|onmouseup|onmouseover|onmousemove|onmouseout|onkeypress|onkeydown|onkeyup|abbr|axis|headers|scope|rowspan|colspan|align|char|charoff|valign|nowrap|bgcolor|width|height]",media_strict:false,s_dropmenu_menus:{}};
switch(a){case"basic_comment":sTinymceOpts.theme_advanced_resizing_min_height=sTinymceOpts.min_height=60;case"basic_update":sTinymceOpts.theme_advanced_buttons1="bold,italic,underline,bullist,numlist,indent,outdent,spellchecker";sTinymceOpts.plugins="spellchecker,paste";break;case"basic_folder_description":sTinymceOpts.theme_advanced_buttons1="bold,italic,underline,bullist,numlist,forecolor,formatting,indent,outdent,justifyleft,justifycenter,justifyright,insert_content,spellchecker";sTinymceOpts.plugins="-s_dropmenu,-s_content,-s_image_hover,spellchecker,media,-paste,inlinepopups";sTinymceOpts.s_dropmenu_menus={insert_content:{title:Drupal.t("Insert Content"),items:sTinymce.getDropMenu("s_insert_image,s_insert_link,s_insert_symbol,s_insert_equation,s_insert_latex")},formatting:{title:Drupal.t("Formatting"),items:sTinymce.getDropMenu("strikethrough,sup,sub,separator,s_format_remove")}};sTinymceOpts.theme_advanced_resizing=false;sTinymceOpts.theme_advanced_statusbar_location="";break;default:case"full":sTinymceOpts.plugins="-s_dropmenu,-s_content,-s_image_hover,-s_cluetip,-s_table,spellchecker,media,-paste,table,inlinepopups";
sTinymceOpts.theme_advanced_buttons1="bold,italic,underline,bullist,numlist,forecolor,backcolor,formatting,indent,outdent,justifyleft,justifycenter,justifyright,s_link,insert_content,s_table_button,spellchecker,formatselect,fontsizeselect";sTinymceOpts.theme_advanced_buttons2="s_table_border,row_before,row_after,delete_row,col_before,col_after,delete_col,merge_cells,split_cells,delete_table";sTinymceOpts.s_dropmenu_menus={insert_content:{title:Drupal.t("Insert Content"),items:sTinymce.getDropMenu("s_insert_imagemedia,s_insert_link,s_insert_symbol,s_insert_equation,s_insert_latex,s_cluetip_toggle_note")},formatting:{title:Drupal.t("Formatting"),items:sTinymce.getDropMenu("strikethrough,sup,sub,separator,s_format_remove")}};break}sTinymceOpts.theme_advanced_font_sizes={};var f=[11,12,13,14,16,18,24];$.each(f,function(g,h){sTinymceOpts.theme_advanced_font_sizes[h]={fontSize:h+"px"}});sTinymceOpts=$.extend({},sTinymceOpts,b,e);tinyMCE.init(sTinymceOpts)};if(typeof sTinymce=="undefined"){var sTinymce=(function(){var b={};
var a={s_insert_imagemedia:{id:"s_insert_imagemedia",title:Drupal.t("Image/Media"),cmd:"sContentMediaInsert"},s_insert_image:{id:"s_insert_image",title:Drupal.t("Image"),cmd:"sContentImageInsert"},s_insert_link:{id:"s_insert_link",title:Drupal.t("Link"),cmd:"mceLink"},s_insert_symbol:{id:"s_insert_symbol",title:Drupal.t("Symbol"),cmd:"mceCharMap"},s_insert_equation:{id:"s_insert_equation",title:Drupal.t("Equation"),cmd:"sContentFormulaInsert"},s_insert_latex:{id:"s_insert_latex",title:Drupal.t("LaTeX"),cmd:"sContentLatexInsert"},s_cluetip_toggle_note:{id:"s_cluetip_toggle_note",title:Drupal.t("Tooltip"),cmd:"sCluetipToggleNote",disabled:true},strikethrough:{id:"strikethrough",title:Drupal.t("Strikethrough"),cmd:"strikethrough"},sup:{id:"sup",title:Drupal.t("Superscript"),cmd:"superscript"},sub:{id:"sub",title:Drupal.t("Subscript"),cmd:"subscript"},separator:{type:"separator"},s_format_remove:{id:"s_format_remove",title:Drupal.t("Clear Formatting"),cmd:"removeformat"}};b.updateRichtextContent=function(d){var f=tinyMCE&&tinyMCE.activeEditor?tinyMCE.activeEditor.editorId:null;
if(f){var g=$("#"+f,d);if(g.length){var e=tinyMCE.get(tinyMCE.activeEditor.editorId).getContent();g.val(e)}}};b.getDropMenu=function(e){var d=[];$.each(e.split(","),function(f,g){if(typeof a[g]!="undefined"){d.push(a[g])}});return d};return b}())}if(typeof sTinymcePopup=="undefined"){var sTinymcePopup=(function(){var a={};a.viewModeHeights={"edit-description-wrapper":{lite:128,standard:228},"edit-post-wrapper":{lite:128,standard:228},"edit-template-fields-body-wrapper":{lite:128,standard:228}};a.setView=function(e,b){var f={standard:true,lite:true},d=this.getPopupContainer();if(d){$.each(f,function(g,k){var h="s-tinymce-view-"+g;if(e==g){d.addClass(h)}else{d.removeClass(h)}});if(typeof b=="undefined"||b){sUserSetUISettings("rte","view_mode",e)}a.updateEditorHeight(e);sPopupsResizeCenter()}};a.getView=function(){var e=null,d=this.getPopupContainer();if(d){var b=/s-tinymce-view-([^\s]+)/.exec(d.attr("class"));if(b.length>1){e=b[1]}}return e};a.updateEditorHeight=function(f){var d=tinyMCE.activeEditor;
if(d){if(typeof f=="undefined"){f=a.getCurrentViewMode()}var e=$(d.contentAreaContainer).closest(".form-item").attr("id"),b=null;if(typeof a.viewModeHeights[e]=="object"&&typeof a.viewModeHeights[e][f]!="undefined"){d.theme.resizeTo(null,a.viewModeHeights[e][f])}}};a.getPopupContainer=function(){var e=$(document).data("sTinymcePopupActiveEditors"),d=null;if(e){var b=Popups.activePopup();d=$("#"+e).parents(".popups-box")}return d};a.getCurrentViewMode=function(){var e=this.getPopupContainer(),b=null,d=null;if(e){b=/s-tinymce-view-(\w+)/.exec(e.attr("class"));if(b){d=b[1]}}return d};return a}())}(function(){var a=tinymce.dom.Event;var b=false;tinymce.create("tinymce.plugins.SImageHoverPlugin",{init:function(d,e){var f=this;d.onInit.add(function(g){f._createBtns();a.add(g.getWin(),"scroll",function(h){if(!tinyMCE.activeEditor){a.remove(g,"scroll");return true}tinyMCE.activeEditor.execCommand("sImageHover_removeModBtns",false,false,{skip_focus:true})})});d.addCommand("sImageHover_removeModBtns",this._removeBtns,this);
d.addCommand("sImageHover_positionModBtns",this._positionBtns,this);d.addCommand("sImageHover_getEnabledImage",this._getEnabledImage,this);d.addCommand("sImageHover_setEnabledImage",this._setEnabledImage,this);$("body").bind("mousedown",function(k){if(typeof tinyMCE=="undefined"||typeof tinyMCE.activeEditor=="undefined"){$(this).unbind(k);return true}var h=$(k.target);var g=String(h.attr("id"));if(g=="undefined"){g=h.parent().attr("id")}if(g=="undefined"||g=="tme_imgModBtn"||g=="tme_imgDelBtn"){return}tinyMCE.activeEditor.execCommand("sImageHover_removeModBtns",false,false,{skip_focus:true})});d.onRemove.add(function(g){$("#tme_imgModEnabled").attr("id","");$("#tme_modBtnContainer").hide()});d.onClick.add(function(g,h){var k=tinyMCE.activeEditor.execCommand("sImageHover_getEnabledImage");if(typeof k=="undefined"||(k!=false&&$(h.target)[0]==k[0])){return}tinyMCE.activeEditor.execCommand("sImageHover_removeModBtns")});d.onMouseUp.add(function(h,m){if(m.target.nodeName=="IMG"){var g=$(m.target);
var k=String(g.attr("class"));var l=$("span",$("#tme_imgModBtn"));var n=k.match(/^.*(mceItemQuickTime|mceItemMedia|mceItemFlash|mceItemWindowsMedia|mceItemRealMedia).*$/gi)?"media":false;if(!n){n=k.match(/^.*tutortrove-formula.*$/gi)?"tt-formula":false}if(!n){n=k.match(/^.*latex-formula.*$/gi)?"latex-formula":false}if(!n){n=k.match(/^.*mathquill-formula.*$/gi)?"mathquill-formula":false}switch(n){case"media":l.css("background-image","url(/sites/all/libraries/tinymce/jscripts/tiny_mce/themes/advanced/img/icons.gif)");l.css({"background-position":"-323px -24px","background-repeat":"no-repeat"});l.data("mceMethod","mceMedia");break;case"tt-formula":l.css("background-image","url(/sites/all/themes/schoology_theme/images/math-icon.png)");l.css({"background-position":"1px 1px","background-repeat":"no-repeat"});l.data("mceMethod","sContentFormulaInsert");l.data("mceParams",g);break;case"latex-formula":l.css("background-image","url(/sites/all/themes/schoology_theme/images/math-icon.png)");l.css({"background-position":"-1px -16px","background-repeat":"no-repeat"});
l.data("mceMethod","sContentLatexInsert");l.data("mceParams",g);break;case"mathquill-formula":l.css("background-image","url(/sites/all/themes/schoology_theme/images/math-icon.png)");l.css({"background-position":"1px 1px","background-repeat":"no-repeat"});l.data("mceMethod","sContentFormulaInsert");l.data("mceParams",g);break;default:l.css("background-image","url(/sites/all/libraries/tinymce/jscripts/tiny_mce/themes/advanced/img/icons.gif)");l.css({"background-position":"-384px -4px","background-repeat":"no-repeat"});l.data("mceMethod","mceImage");break}h.execCommand("sImageHover_setEnabledImage",m.target);h.execCommand("sImageHover_positionModBtns")}})},_getEnabledImage:function(){return this._enabledImage},_setEnabledImage:function(d){this._enabledImage=$(d);this._enabledImage.attr("id","tme_imgModEnabled")},_createBtns:function(){if($("#tme_modBtnContainer").length){return}var f=$("<div></div>");f.attr("id","tme_modBtnContainer");var d=$("<span></span>");d.attr("id","tme_imgModBtn");d.append($("<span></span>"));
d.bind("click",function(){var h=tinyMCE.activeEditor;if(jQuery.browser.msie){h.selection.select(h.dom.select("img#tme_imgModEnabled")[0])}var g=$("span",$(this));h.execCommand("sImageHover_removeModBtns");h.execCommand(g.data("mceMethod"),g.data("mceParams"))});var e=$("<span></span>");e.attr("id","tme_imgDelBtn");e.append($("<span></span>"));e.bind("click",function(){tinyMCE.activeEditor.execCommand("sImageHover_removeModBtns",true)});f.append(d);f.append(e);f.css("display","none");$("body").append(f);f.hide()},_removeBtns:function(d){if(typeof d!="boolean"){d=false}$(this._enabledImage).attr("id","");if(d){tinyMCE.activeEditor.dom.remove(this._enabledImage);tinyMCE.activeEditor.execCommand("sContentSaveLastNode")}if(tinyMCE.activeEditor==null){return}$("#tme_modBtnContainer").hide();this._enabledImage=false},_positionBtns:function(e){if(this._enabledImage==false){return}if(typeof e!="boolean"){e=true}var l=$(this._enabledImage).position();var k=$("iframe",$(tinyMCE.activeEditor.contentAreaContainer));
var d=k.offset();var h=tinyMCE.activeEditor.contentWindow.pageYOffset;if(typeof h=="undefined"){h=tinymce.DOM.getViewPort(tinymce.activeEditor.getWin()).y}var g=d.top+l.top+5-h;if(d.top>g){g=d.top+10}var f=$("#tme_modBtnContainer");f.css("left",String(d.left+l.left+5)+"px");f.css("top",String(g)+"px");if(e){f.show()}},getInfo:function(){return{longname:"S_Image_Hover",author:"Schoology, Inc.",authorurl:"http://www.schoology.com",infourl:"http://www.schoology.com",version:tinymce.majorVersion+"."+tinymce.minorVersion}}});tinymce.PluginManager.add("s_image_hover",tinymce.plugins.SImageHoverPlugin)})();(function(){var a=tinymce.dom.Event;tinymce.create("tinymce.plugins.SContentPlugin",{init:function(b,d){var e=this;b.addCommand("sContentMediaInsert",function(){e._mediaInsert()});b.addCommand("sContentImageInsert",function(){e._mediaInsert({type:"image"})});b.addCommand("sContentFormulaInsert",this._formulaInsert);b.addCommand("sContentLatexInsert",this._latexInsert);b.addCommand("sContentInsert",this._insertContent);
b.addCommand("sContentSaveLastNode",this._saveLastNode);b.addCommand("sContentToggleSubmitBtns",this._toggleSubmitButtons);b.addCommand("sContentGetRealmFromUrl",this._getRealmFromUrl);b.addCommand("sContentSetWmodeOpaque",this._setWmodeOpaque);b.onEvent.add(function(f,g){if(g.type=="submit"){return}tinyMCE.activeEditor.execCommand("sContentSaveLastNode")});b.addButton("sContentFormula",{title:Drupal.t("Equation"),cmd:"sContentFormulaInsert"});b.addButton("sContentImage",{title:Drupal.t("Image/Media"),cmd:"sContentMediaInsert"})},createControl:function(d,b){if(d=="s_link"){return b.createButton("s_link",{title:"advlink.link_desc",cmd:"mceLink"})}},_saveLastNode:function(){var b=tinyMCE.activeEditor.selection.getNode();if(typeof b=="object"&&b.nodeName=="#document"){b=b.body}$(document).data("tinymceLastNode",b)},_mediaInsert:function(e){if(typeof e=="undefined"){e={}}if(typeof e.type=="undefined"){e.type="both"}sAfu.onUpload=function(l){if(!l.url||l.url==""||!l.type){return}if(l.type=="image"){var h='<img src="'+l.url+'" alt="" title="" />';
tinyMCE.activeEditor.execCommand("sContentInsert",h);var g=Popups.activePopup();Popups.close(g)}else{var k=l.url;var m=l.fid?l.fid:"";if(sCommonIsEmbed(k)){Popups.close();k=tinyMCE.activeEditor.execCommand("sContentSetWmodeOpaque",k);tinyMCE.activeEditor.execCommand("sContentInsert",k);return}$.ajax({url:"/media/richtext/embedcode",data:{fid:m,url:k},type:"POST",dataType:"json",success:function(p,o){var n=Popups.activePopup();Popups.close(n);tinyMCE.activeEditor.execCommand("sContentInsert",p.embedCode)},error:function(o,n,p){alert(Drupal.t("There was an internal error. Please try again in a few moments."))}})}};var b=tinyMCE.activeEditor.execCommand("sContentGetRealmFromUrl");var d="/tinymceinsertmedia?r="+String(b[0])+"&id="+String(b[1]);if(e.type=="image"){d+="&type=image"}var f={ajaxForm:false,extraClass:"tinymce-insert-popup",updateMethod:"none",href:d,hijackDestination:false,disableCursorMod:true,disableAttachBehaviors:false,formOnSubmit:function(){if($("#mediaInsertTabURL").hasClass("active")&&$("#edit-submit-upload").hasClass("disabled")){return false
}var h=$("#edit-media-url"),l=$(".edit-media-type.active"),g=h.val();if(g==""){if($("#mediaInsertTabUpload").hasClass("active")){alert(Drupal.t("Please enter either a URL or embed code."));h.focus();tinyMCE.activeEditor.execCommand("sContentToggleSubmitBtns");return false}var k={success:function(n,m){if(n.status!=0){tinyMCE.activeEditor.execCommand("sContentToggleSubmitBtns");alert(Drupal.t("There was an internal error. Please try again in a few moments."));return}sAfu.onUpload(n.file)},error:function(n,m,o){tinyMCE.activeEditor.execCommand("sContentToggleSubmitBtns");alert(Drupal.t("There was an internal error. Please try again in a few moments."))},url:"/tinymceinsertmedia"};sAfu.uploadFormAjax({},k)}else{sAfu.onUpload({type:l.hasClass("edit-media-type-image")?"image":"media",url:g})}return false}};tinyMCE.activeEditor.execCommand("sContentSaveLastNode");Popups.openPath(this,f,window)},_formulaInsert:function(g){var d=tinyMCE.activeEditor.execCommand("sContentGetRealmFromUrl");var k=g?$(g).attr("formula"):"";
var b=true;if(k&&Base64.is_encoded(k)){k=Base64.decode(k)}if(g){$(document).data("s_content_saved_formula_image",g);b=g.hasClass("mathquill-formula")}if(b){var f="/tinymceinsertmathquill";var h=" tinymce-mathquill-formula-popup"}else{var f="/tinymceinsertformula";var h=" tinymce-formula-popup"}var e={ajaxForm:false,extraClass:"tinymce-insert-popup popups-large"+h,width:550,updateMethod:"none",href:f+"?r="+String(d[0])+"&id="+String(d[1])+"&formula="+encodeURIComponent(k),hijackDestination:false,disableCursorMod:true,disableAttachBehaviors:false,formOnSubmit:function(){}};tinyMCE.activeEditor.execCommand("sContentSaveLastNode");Popups.openPath(this,e,window)},_latexInsert:function(e){var b=tinyMCE.activeEditor.execCommand("sContentGetRealmFromUrl");var f=e?$(e).attr("formula"):"";if(e){$(document).data("s_content_saved_latex_image",e)}var d={ajaxForm:false,extraClass:"tinymce-insert-popup tinymce-latex-popup popups-large",width:550,updateMethod:"none",href:"/tinymceinsertlatex?r="+String(b[0])+"&id="+String(b[1])+"&formula="+encodeURIComponent(f),hijackDestination:false,disableCursorMod:true,disableAttachBehaviors:false,formOnSubmit:function(){}};
tinyMCE.activeEditor.execCommand("sContentSaveLastNode");Popups.openPath(this,d,window)},_insertContent:function(m,b){var h={use_native_insert:false};if(typeof b=="undefined"){b={}}b=$.extend({},h,b);var g=typeof m=="string"?m:$("<div />").append(m.clone()).remove().html();var d=$(document).data("tinymceLastNode");var f=$(d);var k=tinyMCE.activeEditor;try{if(!b.use_native_insert&&f.length>0){var n=d.nodeName.toLowerCase();g=new tinyMCE.html.Serializer().serialize(k.parser.parse(g));if(n=="td"||n=="body"){f.append(g)}else{f.after(g)}}else{k.execCommand("mceInsertContent",false,"");k.execCommand("mceInsertContent",false,g)}}catch(l){}},_toggleSubmitButtons:function(){var b=$("#edit-submit-url");if(b.attr("disabled")==true){b.attr("disabled",false);b.parent().removeClass("disabled")}else{b.attr("disabled",true);b.parent().addClass("disabled")}},_getRealmFromUrl:function(){var e=String(window.location.pathname).split("/");var h=["user","school","district","course","group","page","topic","assignment","event","message","scorm","resources"];
var d="",k=null,g=null;var b=String(window.location.pathname).match(/^\/template\/([0-9]+)\/assessment\/(questions|settings|preview)/);if(b){e=["resources",b[1]]}for(var f=0;f<e.length;f++){d=e[f];if($.inArray(d,h)!=-1){k=d;g=e[f+1];break}}return[k,g]},_setWmodeOpaque:function(e){var d=$(e);if(e.match(/^<iframe.+/i)){var f=d.attr("src");if(f.match(/^http(s?):\/\/.*((youtube\.com)|(youtu\.be)|(vimeo\.com)).+/gi)){if(f.match(/^.+(\&|\?).*wmode=.+$/i)){f=f.replace(/^(.+(\&|\?).*wmode=)([^\&]+)(.*)$/i,"$1opaque$4");d.attr("src",f)}else{var b=f.indexOf("?")==-1?"?":"&";d.attr("src",f+b+"wmode=opaque")}}}if(e.match(/^<(object|embed).+/i)){$("param[name=wmode]",d).remove();d.append('<param name="wmode" value="opaque"/>');d.attr("wmode","opaque")}return $("<div />").append(d).html()},getInfo:function(){return{longname:"S_Content",author:"Schoology, Inc.",authorurl:"http://www.schoology.com",infourl:"http://www.schoology.com",version:tinymce.majorVersion+"."+tinymce.minorVersion}}});tinymce.PluginManager.add("s_content",tinymce.plugins.SContentPlugin)
})();(function(){var o="s_cluetip_highlight";var f="."+o;var n=o+"_tmp";var k="."+n;var a=null;var g="-10000px";function l(){var p=tinymce.activeEditor,r=$("#"+p.editorContainer),q=$(".add-note-popup",r);return(q.css("top")!=g)}function h(t){var H=0,q,u=null,A=null,D=null,v=tinymce.activeEditor,z=$("#"+v.editorContainer),F=$(v.contentAreaContainer),s=F.offset().top-z.offset().top,B=$(".add-note-popup",z),r=$(".arrow-n",B),E=B.outerWidth(),C=B.outerHeight(),G=5;t.each(function(J,K){var I=$(K);var L=I.offset();if(!u||L.left<u){u=L.left}if(!A||(L.left+I.outerWidth())>A){A=L.left+I.outerWidth()}if(L.top+I.outerHeight()>H){H=L.top+I.outerHeight()}});D=(u+A)/2;q=D-(E/2);if(q<0){q=G;r.css("margin-left",(D-(r.innerWidth()/2)-G)+"px")}else{if(q+E>F.outerWidth()){q=F.outerWidth()-E-G;r.css("margin-left",(D-q-(r.innerWidth()/2))+"px")}else{r.css("margin-left","47%")}}var p=z.height()-C-r.outerHeight();H=Math.min(H-$(v.contentDocument).scrollTop()+s,p);B.css({left:q+"px",top:H+"px"})}function m(s){var q=tinymce.activeEditor,t=$("#"+q.editorContainer),r=$(".add-note-popup",t);
var p=s.attr("cluetip");if(Base64.is_encoded(p)){p=Base64.decode(p)}$(".cluetip_text",r).val(p);$(".check-mark-icon",r).unbind("mousedown").bind("mousedown",function(v){v.stopPropagation();var u=$(".cluetip_text",r).val();if(u.length>0){u=Base64.encode(u);s.each(function(z,A){$(A).attr("cluetip",u).removeClass(n)})}a=null;d()});if(p&&p.length>0){$(".delete-icon",r).unbind("mousedown").bind("mousedown",function(u){u.stopPropagation();e(s);a=null;d()}).show()}else{$(".delete-icon",r).unbind("mousedown").hide()}h(s)}function e(q){var p=tinymce.activeEditor.formatter;q.each(function(u,v){var r=$(v),t=r.children(),s=r.find(k);if(t.length&&!s.length){r.replaceWith(r.html())}else{p.toggle("highlight",null,r.get(0))}})}function d(){var p=tinymce.activeEditor,q=$(".add-note-popup",$("#"+p.editorContainer));q.css("top",g);q.find(".cluetip_text").val("");e($(k,$(p.contentDocument)))}function b(){var p=tinymce.activeEditor,r=$(".add-note-popup",$("#"+p.editorContainer)),q=r.find(".cluetip_text").val();
if(q&&q.length>0){$(".check-mark-icon",r).trigger("mousedown")}}tinymce.create("tinymce.plugins.SCluetipPlugin",{init:function(p,q){p.addCommand("sCluetipToggleNote",this._toggleNote);p.onNodeChange.add(this._nodeChanged,this);p.onBeforeGetContent.add(this._beforeGetContent,this);p.onInit.add(function(t){t.formatter.register({highlight:{inline:"span",classes:[o,n],attributes:{cluetip:""},onmatch:function(z,B,C){var A=$(z);return(A.hasClass(o)&&A.hasClass(n))}}});var v=$("#"+t.editorContainer);var s=$("<div></div>").css("position","absolute").addClass("add-note-popup").prependTo(v);$("<div></div>").addClass("arrow-n").appendTo(s);var r=$("<div></div>").addClass("add-note-popup-content").appendTo(s);$('<input type="text"/>').addClass("cluetip_text").appendTo(r);$("<span></span>").addClass("inline-icon mini check-mark-icon").appendTo(r);$("<span></span>").addClass("inline-icon mini delete-icon").appendTo(r);var u=v.closest(".popups-box");if(u.length>0){u.addClass("allow-overflow")}d()})},_toggleNote:function(){var q=tinymce.activeEditor,s=$(q.selection.getNode()),p=s.parents(f),r=null;
if(s.hasClass(o)){e(s)}else{if(p.length>0){p.eq(-1);e(p)}else{q.execCommand("mceToggleFormat",false,"highlight");if(!s.is(":visible")){s=$(q.selection.getNode())}r=s.find(k);if(!r.length){r=s.addClass(n).addClass(o)}m(r)}}},_nodeChanged:function(t,A,r,z,q){var p=$(r);var u=p.hasClass(o);var v=p.parents(f);var B=u||v.length>0;if(c=A.get("s_cluetip_toggle_note")){c.setDisabled(!B&&z);c.setActive(B)}if(B){var s=u?p:v;if(a!=r){b();m(s)}}else{if(l()){b();d()}}a=r},_beforeGetContent:function(p){var q=$(".add-note-popup",$("#"+p.editorContainer));if(q.length>0&&l()){b();d()}},getInfo:function(){return{longname:"S_Cluetip",author:"Schoology, Inc.",authorurl:"http://www.schoology.com",infourl:"http://www.schoology.com",version:tinymce.majorVersion+"."+tinymce.minorVersion}}});tinymce.PluginManager.add("s_cluetip",tinymce.plugins.SCluetipPlugin)})();(function(){var a={};tinymce.create("tinymce.plugins.SDropMenu",{init:function(b,d){var e=this;a=typeof b.settings.s_dropmenu_menus=="object"?b.settings.s_dropmenu_menus:{}
},createControl:function(g,b){var f=this;if(typeof a[g]!="undefined"){var d=a[g],e=null;e=b.createMenuButton(g,{title:d.title,label:d.label,icons:false});e.onRenderMenu.add(function(k,h){f._createSubMenu(k.editor,h,d.items)});e.renderMenu();return e}},_createSubMenu:function(d,f,b){var e=this;$.each(b,function(g,n){if(typeof n.items!="undefined"){var m=f.addMenu({title:n.title});e._createSubMenu(d,m,n.items)}else{if(typeof n.type!="undefined"){if(n.type=="separator"){f.addSeparator()}}else{var h={title:n.title},l=null;if(typeof n.onclick=="function"){h.onclick=function(k){n.onclick(k,d)}}else{if(typeof n.cmd=="string"){h.cmd=n.cmd}}if(typeof n.id!="undefined"){h.id=n.id}l=f.add(h);if(typeof n.disabled!="undefined"&&n.disabled){l.setDisabled(true)}d.controlManager.add(l)}}})},getInfo:function(){return{longname:"S_DropMenu",author:"Schoology, Inc.",authorurl:"http://www.schoology.com",infourl:"http://www.schoology.com",version:tinymce.majorVersion+"."+tinymce.minorVersion}}});tinymce.PluginManager.add("s_dropmenu",tinymce.plugins.SDropMenu)
})();(function(){var a={};function b(k){var l=null,f=4,p=4,o=15,m=10,h=25,n=17;if(typeof a[k.id]=="object"&&a[k.id].closest("body").length){l=a[k.id]}else{l=a[k.id]=$("#menu_"+k.id+"_co")}if(!l.children(".table-picker-wrapper").length){var e='<div class="table-picker-wrapper"><div class="table-picker-cells"><div class="picker table-picker-selected-cell"></div></div><p class="table-picker-desc"></p></div>';l.append(e);var g=l.find(".table-picker-selected-cell"),d=l.find(".table-picker-cells");l.resetSelection=function(){l.updateSelection(1,1)};l.getColsRows=function(s,r){var v=d.offset(),q=s-v.left,z=r-v.top,u=Math.min(Math.max(Math.ceil(q/h),1),m),t=Math.min(Math.max(Math.ceil(z/n),1),o);return[u,t]};l.updateSelection=function(t,s){var r=Math.max(t,p),q=Math.max(s,f);g.css({width:t*h+1,height:s*n+1});d.css({width:r*h+1,height:q*n+1});l.find(".table-picker-desc").text(t+" x "+s)};d.bind("mousemove.tablepicker",function(r){r.stopPropagation();var q=l.getColsRows(r.pageX,r.pageY);l.updateSelection(q[0],q[1])
}).bind("mouseleave.tablepicker",function(q){$("body").bind("mousemove.tablepicker",function(s){if(l.is(":visible")){var r=l.getColsRows(s.pageX,s.pageY);l.updateSelection(r[0],r[1])}})}).bind("mouseenter.tablepicker",function(q){$("body").unbind("mousemove.tablepicker")}).bind("click.tablepicker",function(t){var s=l.getColsRows(t.pageX,t.pageY),r=0,q='<table class="s_table s_table_border">';for(r=0;r<s[1];r++){q+="<tr>";for(j=0;j<s[0];j++){q+="<td><br/></td>"}q+="</tr>"}q+="</table>";tinyMCE.activeEditor.execCommand("mceInsertContent",false,q);k.hideMenu()})}if(typeof l.resetSelection=="function"){l.resetSelection()}}tinymce.create("tinymce.plugins.STablePlugin",{init:function(e,f){var g=this,d=null,k=null,h=null;e.addCommand("mceTablePicker",function(){if(k){var l=tinyMCE.activeEditor.controlManager;button=$("#"+k.id),offset=button.offset(),x=offset.left,y=offset.top+button.height();if(d.isMenuVisible){d.hideMenu()}else{d.showMenu(x,y);b(d)}}});e.addCommand("mceToggleTableBorder",this.toggleTableBorder,this);
e.onInit.add(function(){var l=e.controlManager;k=l.get(l.prefix+"s_table_button");d=l.createDropMenu("createtable");d.onHideMenu.add(function(){$("body").unbind("mousemove.tablepicker");tinyMCE.dom.Event.unbind(tinyMCE.DOM.doc,"mousedown")});d.onShowMenu.add(function(){tinyMCE.dom.Event.bind(tinyMCE.DOM.doc,"mousedown",function(m){if(!$(m.target).closest("#"+k.id+", #menu_"+d.id).length){d.hideMenu()}})});h=$("#"+e.editorId+"_delete_table").closest(".mceToolbar");if(h){g.isVisible=false;g._resizeIframe(e,31);sPopupsResizeCenter()}});e.onNodeChange.add(function(m,l,t,r,q){var p=m.dom.getParent(m.selection.getStart(),"table");if(k){k.setActive(p)}if(h){if(p){if(!g.isVisible){g.isVisible=true;h.show();g._resizeIframe(m,-31)}}else{if(g.isVisible){g.isVisible=false;h.hide();g._resizeIframe(m,31)}}}if(c=l.get("s_table_border")){var s=$(p);c.setActive(s.hasClass("s_table_border"));c.setDisabled(!p)}});e.onClick.add(function(){if(d){d.hideMenu()}});g.isVisible=false},toggleTableBorder:function(){var d=tinymce.activeEditor,e=$(d.dom.getParent(d.selection.getStart(),"table"));
e.toggleClass("s_table_border")},createControl:function(g,d){switch(g){case"s_table_button":var f=d.createButton(g,{title:"table.desc","class":"mce_table",onclick:function(){tinymce.activeEditor.execCommand("mceTablePicker")}});return f;case"s_table_border":var e=d.createButton(g,{title:Drupal.t("Border"),"class":"mce_table_border",cmd:"mceToggleTableBorder"});return e}return null},_resizeIframe:function(d,f){var e=$(d.getContentAreaContainer().firstChild);e.height(e.height()+f);d.theme.deltaHeight+=f}});tinymce.PluginManager.add("s_table",tinymce.plugins.STablePlugin)})();